...
def index():
    ...
    user_portfolio = db.execute(
        "SELECT id, symbol, name, SUM(shares)  FROM trades WHERE id = ? GROUP BY symbol HAVING SUM(shares) > 0 ORDER BY price DESC",
        session["user_id"],
    )
...
def history():
    ...
    user_transactions = db.execute(
        "SELECT id, symbol, shares, price, transacted  FROM trades WHERE id = ? ORDER BY transacted",
        session["user_id"],
    )
...
def sell():
    ...
    if request.method == "POST":
        ...
        user_portfolio = db.execute(
            "SELECT id, symbol, SUM(shares) FROM trades WHERE id = ? AND symbol = ? GROUP BY symbol HAVING SUM(shares)>0 ",
            session["user_id"],
            stock_quote["symbol"],
        )
        user_cash = db.execute("SELECT * FROM users WHERE id = ?", session["user_id"])
    ...
    # User reached route via GET
        ...
        user_portfolio = db.execute(
            "SELECT id, symbol, SUM(shares) FROM trades WHERE id = ? GROUP BY symbol HAVING SUM(shares)>0 ORDER BY symbol",
            session["user_id"],
        )
...
def passwordupdate():
    ...
    if request.method == "POST":
        ...
        if not check_password_hash(rows[0]["hash"], currentpassword):
            ...
            db.execute(
                "UPDATE users SET hash = ? WHERE id = ?", hashcode, session["user_id"]
            )
